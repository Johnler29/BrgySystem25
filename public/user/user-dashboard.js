...
async function initStats(){
  try{
    ...
    const myDocs = userDocs.length;
    const pending = userDocs.filter(d=>d.status==='Pending').length;
    const released = userDocs.filter(d=>d.status==='Released').length;
    
    $('myDocuments') && countUp($('myDocuments'), myDocs);
    $('pendingRequests') && countUp($('pendingRequests'), pending);
    $('releasedDocs') && countUp($('releasedDocs'), released);
    
    // Get user's cases
    try {
      const cases = await fetchJSON('/api/cases');
      const myCases = (cases.cases||[]).length;
      $('myCases') && countUp($('myCases'), myCases);
    } catch(e) { console.warn('[initStats/cases]', e); }
    
    // Update document status
    if (pending > 0) {
      $('docStatus') && ($('docStatus').textContent = `${pending} Pending`);
    } else if (released > 0) {
      $('docStatus') && ($('docStatus').textContent = `${released} Released`);
    } else {
      $('docStatus') && ($('docStatus').textContent = 'No requests');
    }
  }catch(e){
...
async function initActivities(){
  try{
    ...
    if (!wrap.children.length) {
      wrap.innerHTML = '<div class="announce-row"><div>No recent activities</div></div>';
    }
  }catch(e){
    if (e.code==='UNAUTH' || e.code==='NONJSON'){ location.href='/login'; return; }
...
function initUserMenus(){
  ...
  on($('goSettings'),'click',()=>location.href='/user/settings');
